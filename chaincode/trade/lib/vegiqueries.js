...
class VegiQuery {
    ...
    /**
     * Get Stock History for a specified box.
     * @param boxID
     * @param owner
     */
    async getStockHistory(boxID, owner) {
        let ledgerKey = await this.ctx.stub.createCompositeKey(this.name, [boxID, owner]);
     ...
     * @param BoxID
     */
    async queryKeyByFirstPartOfKey(BoxID) {
        ...
        const resultsIterator = await this.ctx.stub.getStateByPartialCompositeKey(this.name, [BoxID]);
